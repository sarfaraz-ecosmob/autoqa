...
This is a hands-on tour of AutoQA's 12 project stages, each shown with a real screenshot from the bundled **demo-app** (a deliberately flawed test target at `http://demo-app:9000`). Follow along with your own instance at `http://localhost:8080`.
...
```
Overview → Discovery → APIs → Test Plan → Test Cases → Executions
   → History → Quality → Assistant → Test Data → Reports → Automations
```
...
![Discovery — sitemap and scan controls](img/02-discovery.png)
...
![APIs — grouped endpoint inventory](img/03-apis.png)
...
![History — compare runs](img/07-history.png)
...
![Quality — a11y + performance results](img/08-quality.png)
...
![Assistant — grounded QA summary](img/09-assistant.png)
...
![Test data — environments, datasets, resolve preview](img/10-test-data.png)
...
![Reports — generation & history](img/11-reports.png)
...
![Automations — schedules, webhooks, flaky, visual](img/12-automations.png)

---

## The loop

Stages 1–6 are the core loop you run per release; 7–12 make QA continuous. Typical cadence:

1. Set up once: credentials, environments, datasets, schedule, webhook.
2. Per release: **Discovery** (if the app changed) → **Executions** (or let the schedule fire) → **History** compare against the last good run.
3. Share: **Reports** for stakeholders; **Assistant** for quick answers; **Automations** keeps it all running nightly.

For the stage semantics in more depth, see the [stage-by-stage guide in the README](../README.md#stage-by-stage-guide-project-tabs). For credentials, auth flows and payment testing, see [Authenticated targets](../README.md#authenticated-targets-credentials-portals--payment-flows).
